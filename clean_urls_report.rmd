...
name_links_dt="all_links.csv"#"all_links.csv"
url_links=rio::import(paste0("files/IMF_urls_raw/",name_links_dt))
url_links=clean_IMF_urls(url_links)

IMF_programs=rio::import("IMF_programs_dates.xlsx")
IMF_links_old=rio::import("IMF_Requests_1960_2014_urls.xlsx")
```
...
urls_clean=urls_clean %>% mutate(type_doc=ifelse(! is.na(type_doc) & (str_detect(title,"prolongation") | str_detect(title,"extension") | str_detect(title,"lengthening")),"extension",type_doc))
...
urls_clean=urls_clean %>% mutate(type_program=ifelse(str_detect(keywords,"Extended fund facility"),"EFF",
                                                                                        ...
                                                                                        ifelse(str_detect(keywords,"Precautionary and Liquidity Line") | str_detect(keywords,"Precautionary Credit Line") ,"PLL",
                                                                                               ifelse(str_detect(keywords,"SAF arrangement"),"SAF",
                                                                                                      ifelse(str_detect(keywords,"Stand-by arrangement") | str_detect(keywords,"Stand-By Arrangements"),"SBA",
                                                                                                             ifelse(str_detect(keywords,"Extended arrangement"),"EA","Other"))))))))))
...
urls_clean=urls_clean %>% 
  dplyr::select(iso3c,date,hierarchy,type_program,type_doc,Review_number,performance_criteria,waiver,everything()) %>%
  arrange(iso3c,date)
...
a=urls_clean %>% filter(str_detect(title,"flexible") & type_hierarchy=="Clean")
