...
```{r,eval=F}
 ...
 urls_clean %>% group_by(iso3c,type_hierarchy) %>%
   summarize(n=n()) %>%
   arrange(iso3c,type_hierarchy) %>%
   mutate(p=dplyr::lag(n,2)/(n+dplyr::lag(n,2))) %>% 
   arrange(-p)
   #filter(type_hierarchy=="Correction")

a=urls_clean %>% filter(iso3c=="PHL")
#View(a)
```
