...
urls_clean=url_links %>% filter(type_hierarchy %in% c("Clean","Supplement")) #select links corresponding to EBS with correction or supplements
...
urls_clean=urls_clean %>%  filter(str_detect(keywords,"Stand-by arrangement reviews") &
                                    !str_detect(keywords,"Press Releases") & 
                                    !str_detect(keywords,"ARTICLE VIII")
                                  )
...
urls_clean=urls_clean %>%  filter(str_detect(title,"review"))
...
urls_clean=urls_clean %>% mutate(Review_number=ifelse(str_detect(title,"first"),"review_1",
                                                      ifelse(str_detect(title,"second"),"review_2",
                                                             ifelse(str_detect(title,"third"),"review_3",
                                                                    ifelse(str_detect(title,"fourth"),"review_4",
                                                                           ifelse(str_detect(title,"fifth"),"review_5",
                                                                                  ifelse(str_detect(title,"sixth"),"review_6",
                                                                                         ifelse(str_detect(title,"seventh"),"review_8",
                                                                                                ifelse(str_detect(title,"eight"),"review_9",
                                                                                                       ifelse(str_detect(title,"ninth"),"review_10",
                                                                                                              ifelse(str_detect(title,"tenth"),"review_11",NA)))))))))))

urls_clean=urls_clean %>% mutate(Review_number=ifelse(str_detect(title,"midterm"),"review_midterm",Review_number),
                                 Review_number=ifelse(str_detect(title,"review") & is.na(Review_number),"review",Review_number))

urls_clean=urls_clean %>% arrange(iso3,date)

names(urls_clean)
urls_clean=urls_clean %>% dplyr::select(iso3,date,hierarchy,Review_number,everything())
...
rio::export(IMF_details_program,"files/IMF_keywords/IMF_keywords.RData")
...
rio::export(urls_clean,"files/IMFSBA_Reviews_links_clean.Rdata")

#year((urls_clean %>% arrange(date))$date) %>% unique()
